---
title: "MSstats simulation 2 msqrob all comparisons with peptidoform analysis"
...
for (i in c(1:8)){
  ...
  #pepform_df <- pivot_wider(simulation2_data[[i]]$PTM, id_cols = c("ProteinName", "PeptideSequence"), names_from = "Run", values_from = "Intensity")
  ...
  group1runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_1") %>% pull(Run) %>% unique()
  ...
  for (protein in unique(rowData(pe)[["peptidoform"]]$PROTEIN)){
    pe_sub <- pe[["peptidoform"]][rowData(pe[["peptidoform"]])$PROTEIN %in% protein,]
  ...
  L <- makeContrast(c("groupG2 = 0"), 
                    parameterNames = rowData(pe[["peptidoformNorm"]])$msqrobModels[[5]] %>%
                  getCoef %>%
                  names)
...
```{r}
print(results_peptidoform)
print(results)
```


```{r}
save(results_peptidoform, file = "results_simulation2_msqrob_comparison1_peptidoform.rda")
save(sign_table_peptidoform, file = "signtable_simulation2_msqrob_comparison1_peptidoform.rda")
```

```{r}
write.csv2(results_peptidoform, file = "results_simulation2_msqrob_comparison1_peptidoform.csv", sep = ";")
```
...
for (i in 9:16){
  ...
  ## We need to know which groups correspond to which runs
  group1runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_1") %>% pull(Run) %>% unique()
  group2runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_2") %>% pull(Run) %>% unique()
  colData(pe)$run <- paste0(rep("run",length(ecols)), sapply(str_split(rownames(colData(pe)), "_"), function(x) x[2]))
...
```{r}
save(results_peptidoform, file = "results_simulation2_msqrob_comparison2_peptidoform.rda")
save(sign_table_peptidoform, file = "signtable_simulation2_msqrob_comparison2_peptidoform.rda")
```
...
for (i in 17:24){
  ...
  group1runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_1") %>% pull(Run) %>% unique()
  group2runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_2") %>% pull(Run) %>% unique()
  group3runs <- simulation2_data[[i]]$PTM %>% filter(Condition == "G_3") %>% pull(Run) %>% unique()
  colData(pe)$run <- paste0(rep("run",length(ecols)), sapply(str_split(rownames(colData(pe)), "_"), function(x) x[2]))
  colData(pe)$group <- case_when(
                        rownames(colData(pe)) %in% group1runs ~ "G1", 
                        rownames(colData(pe)) %in% group2runs ~ "G2",
...
```{r}
save(results_peptidoform, file = "results_simulation2_msqrob_comparison3_peptidoform.rda")
write.csv2(results_peptidoform, file = "results_simulation2_msqrob_comparison3_peptidoform.csv")
save(sign_table_peptidoform, file = "signtable_simulation2_msqrob_comparison3_peptidoform.rda")
```
