---
title: "visualisation simulation analyses peptidoform level msqrob"
output:
  html_document:
    code_download: yes
    theme: cosmo
...
```{r}
#msqrob data
load("simulation 1/signtable_simulation1_msqrob_comparison1_peptidoform.rda")
sign_table11 <- sign_table_peptidoform
load("simulation 1/signtable_simulation1_msqrob_comparison2_peptidoform.rda")
sign_table12 <- sign_table_peptidoform
load("simulation 1/signtable_simulation1_msqrob_comparison3_peptidoform.rda")
sign_table13 <- sign_table_peptidoform

load("simulation 2/signtable_simulation2_msqrob_comparison1_peptidoform.rda")
sign_table21 <- sign_table_peptidoform
load("simulation 2/signtable_simulation2_msqrob_comparison2_peptidoform.rda")
sign_table22 <- sign_table_peptidoform
load("simulation 2/signtable_simulation2_msqrob_comparison3_peptidoform.rda")
sign_table23 <- sign_table_peptidoform
```
...
model_df2_together %>%
      ...
      geom_path(aes(linetype = model), size = 1) +
      geom_vline(xintercept=0.05,lty=2) +
      geom_point(data = point_data_together_2, aes(x = x, y = y, shape = model), size = 1.5, color = "black") +
      facet_grid(vars(sd), vars(conditions), labeller = label_both)
```
...
pvals <- c()
for (i in 1:length(adjusted_models_sim1)){
  pvals <- c(pvals, model_df %>%
    filter(grepl(pattern = "NoChange", x = Protein)) %>%
    pull(pval))
